...
//       // Only enable fullscreen on mobile
...
export const ProjectPopup = ({ project, onClose }) => {
  ...
  const handleImageClick = (e) => {
    ...
    if (window.innerWidth <= 768) {
      ...
      // Enable click-based navigation on desktop
      const rect = e.currentTarget.getBoundingClientRect();
      const clickX = e.clientX - rect.left;

      if (clickX < rect.width / 2) {
        prevImage(e); // Clicked left side → Previous Image
      } else {
        nextImage(e); // Clicked right side → Next Image
      }
    }
  };
  ...
  const handleTouchEnd = (e) => {
    ...
    if (touchStartX - touchEndX > 50) {
      nextImage(e); // Swipe left → next image
    } else if (touchEndX - touchStartX > 50) {
      prevImage(e); // Swipe right → previous image
    }
  };
